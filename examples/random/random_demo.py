...
def runClustering(dists, quivs, i, j, c, L):
...
if __name__=='__main__':
    ...
    for c in range(C):
        for i in range(N.shape[0]):
            for j in range(N.shape[1]):
                ress.append(pool.apply_async(genSpace, (i,j,c,N,T,P)))
    ...
    ress = []
    for i,j,c,M,Q in MQ:
        ress.append(pool.apply_async(runClustering, (M.dists, Q.quivers, i, j, c, L)))
